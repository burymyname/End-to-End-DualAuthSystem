...
import org.nuaa.speakerverification.CommonUtils;
...
public class BitFrequencyConverter {
    ...
    public BitFrequencyConverter(int startFrequency, int endFrequency, int numberOfBitsInOneTone){
        ...
        //HandshakeStart and HandshakeEnd are highest two frequencies
    ...
    public ArrayList<Integer> calculateFrequency(byte[] byteArray){
        ...
        for(int i=0; i<byteArray.length; i++){
            ...
            for(int j=7; j>=0; j--){
                ...
                if(((currShift+j+1+(byteArray.length-(i+1))*8)<numberOfBitsInOneTone)
                        && (!isDataModulo)){
